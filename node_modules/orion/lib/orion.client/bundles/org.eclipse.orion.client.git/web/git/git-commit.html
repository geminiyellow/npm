<!doctype html>
<html>
<head>
	<meta name="copyright" content="Copyright (c) IBM Corporation and others 2011, 2014." >
	<meta http-equiv="Content-Language" content="en-us">
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<title>Git Commit</title>
	<link rel="stylesheet" type="text/css" href="git-commit.css" />
	<link rel="icon" href="/images/repository.png" type="image/png">
	<script type="text/javascript" src="../requirejs/require.min.js"></script>
	<script type="text/javascript">
	/*globals require*/
		require({
		  baseUrl: '..',
		  // set the paths to our library packages
		  packages: [],
		  paths: {
		    text: 'requirejs/text',
		    i18n: 'requirejs/i18n',
		    domReady: 'requirejs/domReady'
		  }
		});
		require(["git/git-commit"]);
		</script>
</head>
	<body id="orion-git-commit" class="orionPage">
		<div id="sideMenu" class="sideMenu"></div>
		<div id="pageContent" class="content-fixedHeight">
			<div id="sideMenuToggle" class="sideMenuToggle"></div>
			<div class="fixedToolbarHolder mainpane mainPanelLayout">
				<div class="mainToolbar toolComposite" id="pageToolbar"></div>
				<div id="mainNode" class="toolbarTarget pageLayoutTarget">
					<div id="table" class="gitPanel displayTable">
					</div>
				</div>
			</div>
		</div>
		<div class="footer-fixed-bottom footer" id="footer"></div>
	</body>

</html>